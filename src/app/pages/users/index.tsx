...
export const UsersIndex: FC = () => {
  ...
  const [choosenItem, setChoosenItem] = useState<UserType | null>(null);
  ...
  async function getData() {
    try {
      const res = await apiClient.get(`/users${buildQueryParams()}`);
      setData(res.data.data || []);
      setTotalPageCount(res.data.pagination.total);
      setIsLoading(false);
    } catch (error) {
          ...
          (column) =>
            column.isActive && (
              ...
              >
                {renderColumnData(
                  column.key,
                  value[column.key as keyof UserType]
                )}
              </td>
            )
        )}
  ...
  // OTHER
  const changeSelectedItems = (data: UserType[]) => {
    if (data.length <= selectedItems.length) {
      setSelectedItems([]);
      return;
    }
    setSelectedItems(data.map((item: UserType) => item.id));
  };

  const changeQueryParams = (param: QueryParamsType) => {
