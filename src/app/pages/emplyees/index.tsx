...
export const EmployeesIndex: FC = () => {
  const intl = useIntl();
  ...
  const [choosenItem, setChoosenItem] = useState<EmployeeType | null>(null);
  ...
  async function getData() {
    try {
      const res = await apiClient.get(`/employees${buildQueryParams()}`);
  ...
  async function getDataList() {
    try {
      const res = await apiClient.get(`/teams?page_size=200`);
      const newData = res.data.data.map((item: TeamType) => ({
        value: item.id,
        label: item.name,
      }));
      setTeams(newData);
  ...
  const [tableThead] = useState([
    ...
    {
      title: intl.formatMessage({ id: "COMMON.FULL_NAME" }),
      key: "full_name",
      isActive: true,
      disabled: true,
    },
    {
      title: intl.formatMessage({ id: "COMMON.TEAM" }),
      key: "team_id",
      isActive: true,
      disabled: true,
    },
    ...
    {
      title: intl.formatMessage({ id: "COMMON.COUNT_OF_PLAN" }),
      key: "count_of_plan",
      isActive: true,
      disabled: true,
    },
          ...
          (column) =>
            column.isActive && (
              ...
              >
                {renderColumnData(
                  column.key,
                  value[column.key as keyof EmployeeType]
                )}
              </td>
            )
        )}
  ...
  const renderColumnData = (
    key: string,
    value: EmployeeType[keyof EmployeeType]
  ): ReactNode => {
    switch (key) {
      case "full_name":
        return (
  ...
  // OTHER
  const changeSelectedItems = (data: EmployeeType[]) => {
    if (data.length <= selectedItems.length) {
      setSelectedItems([]);
      return;
    }
    setSelectedItems(data.map((item: EmployeeType) => item.id));
  };
